...
# Universal Dotfiles Validation Script
# Run this script to validate your dotfiles configuration

set -e

echo "🔍 Validating universal dotfiles configuration..."
...
for dir in zsh nvim wezterm p10k scripts; do
  if [ -d "$dir" ]; then
...
echo "🖥️  Checking platform-specific configurations..."
if [ -f "scripts/setup-macos.sh" ] && [ -f "scripts/setup-linux.sh" ]; then
  print_status 0 "Platform-specific setup scripts exist"
else
  print_status 1 "Missing platform-specific setup scripts"
fi
...
echo "1. Run './scripts/setup-$PLATFORM.sh' to install prerequisites"
echo "2. Run 'stow .' to install your dotfiles"
